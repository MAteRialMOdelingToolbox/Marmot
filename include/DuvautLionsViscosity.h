...
namespace bft {
    template <int nMatTangentSize> class DuvautLionsViscosity {
      private:
      ...
      public:
        ...
        Vector6 applyViscosityOnStress( const Vector6& trialStress,
                                        const Vector6& stressInf,
                                        double         dT );
        TangentSizedMatrix applyViscosityOnMatTangent( const TangentSizedMatrix& matTangentInv,
                                                       double                    dT );
    };
} // namespace bft
...
namespace bft {
    ...
    template <int s>
    typename DuvautLionsViscosity<s>::TangentSizedMatrix DuvautLionsViscosity<
        s>::applyViscosityOnMatTangent( const TangentSizedMatrix& matTangentInv, double dT ) {
        return ( 1 / ( 1 + dT / viscosity ) ) *
               ( TangentSizedMatrix::Identity() + dT / viscosity * matTangentInv );
    }
} // namespace bft
