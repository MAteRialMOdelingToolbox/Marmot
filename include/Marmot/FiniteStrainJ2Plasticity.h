...
namespace Marmot::Materials {
  ...
  public:
    ...
    {
      const Tensor33t< T > dev = ContinuumMechanics::Micropolar::GeneralizedInvariants::deviatoric( mandelStress );
    ...
    VectorXt< T > computeResidualVector( const VectorXt< T >& X, const Tensor33d& FeTrial, const double alphaPTrial )
    {
      ...
      VectorXt< T > R( 11 );
      // initialize residual
      /* R.segment< 9 >( 0 ) = -mV9t( fastorTensorFromDoubleTensor< T >( FeTrial ).data() ); */
      R( 9 ) = -alphaPTrial;
      ...
      const Tensor33t< T > dFp = ContinuumMechanics::Micropolar::Plasticity::FlowIntegration::exponentialMap( dGp );
