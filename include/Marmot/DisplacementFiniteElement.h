...
namespace Marmot::Elements {
    ...
    DisplacementFiniteElement< nDim, nNodes >::DisplacementFiniteElement(
        ...
        : ParentGeometryElement(),
          ...
          sectionType( sectionType )
    {
        for ( const auto& gaussPtInfo : FiniteElement::Quadrature::getGaussPointInfo( this->shape, integrationType ) ) {
    ...
    void DisplacementFiniteElement< nDim, nNodes >::assignStateVars( double* stateVars, int nStateVars )
    {
         ...
         * possible alternatively: int nStateVarsMaterial = gaussPts[0].material->getNumberOfRequiredStateVars();
         * */
        ...
        for ( size_t i = 0; i < gaussPts.size(); i++ ) {
            ...
            new ( &gpt.stress )
                mVector6d( stateVars + nStateVarsMaterial + i * ( nStateVarsMaterial + GaussPt::nRequiredStateVars ),
                           6 );

            new ( &gpt.strain ) mVector6d( stateVars + nStateVarsMaterial +
        ...
        for ( GaussPt& gaussPt : gaussPts ) {
            ...
            if constexpr ( nDim == 1 ) {
                ...
                if ( sectionType == SectionType::Solid ) {
                    ...
                    gaussPt.material->computeStress( gaussPt.stress.data(), C.data(), dE.data(), time, dT, pNewDT );
                }
    ...
    template < int nDim, int nNodes >
    void DisplacementFiniteElement< nDim, nNodes >::computeDistributedLoad(
