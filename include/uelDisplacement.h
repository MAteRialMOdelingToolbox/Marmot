...
void UelDisplacement<nDim, nNodes>::setInitialConditions(StateTypes state, const double* values)
{
    ...
    switch(state){
        ...
        case BftUel::GeostaticStress: { for(int i = 0; i < gaussPointList.rows(); i++) {

                                          using namespace bft::FiniteElement;
                                          using namespace bft::FiniteElement::Spatial2D::Quad4;
                                                    
                                            MatrixXd NAtGauss = this->NB( N( gaussPointList.row(i)) ); 
                                            Vector2d coordAtGauss =  NAtGauss* this->coordinates; 
