...
namespace {
  ...
  const double mu = 1500.;

  IncompressibleNeoHooke::ConstitutiveResponse< 3 > computeResponse(
    const Tensor33d&                                F,
    IncompressibleNeoHooke::AlgorithmicModuli< 3 >* tangentOut = nullptr )
  {
    const IncompressibleNeoHooke mat( &mu, 1, 1 );

    IncompressibleNeoHooke::Deformation< 3 >          def     = { F };
    IncompressibleNeoHooke::TimeIncrement             timeInc = { 0, 0.1 };
    IncompressibleNeoHooke::ConstitutiveResponse< 3 > response( Tensor33d( 0.0 ), 0.0, 0.0, nullptr );
    IncompressibleNeoHooke::AlgorithmicModuli< 3 >    tangent = { Tensor3333d( 0.0 ) };
...
// Test N-1: at the undeformed configuration, the isochoric stress must vanish.
void testUndeformedResponse()
{
  ...
  throwExceptionOnFailure( checkIfEqual( response.tau, Tensor33d( 0.0 ), 1e-10 ),
                           "N-1: Undeformed configuration must yield zero stress" );
}

// Test N-2: classical closed-form incompressible uniaxial tension (Treloar): for
// F = diag(lambda, lambda^-1/2, lambda^-1/2) (J=1), tau_1 - tau_2 = mu*(lambda^2 - lambda^-1).
void testIncompressibleUniaxialTension()
{
  for ( const double lambda : { 0.7, 0.9, 1.0, 1.2, 1.5 } ) {
    ...
    const double A = mu * lambda * lambda;
    const double B = mu / lambda;
    ...
    throwExceptionOnFailure( checkIfEqual( response.tau, stressTarget, 1e-8 ),
                             "N-2: Incompressible uniaxial tension closed-form check failed for lambda=" +
                               std::to_string( lambda ) );

    throwExceptionOnFailure( std::abs( Fastor::trace( response.tau ) ) < 1e-8,
                             "N-2: Kirchhoff stress must be trace-free (purely isochoric) for lambda=" +
                               std::to_string( lambda ) );
  }
}

// Test N-3: algorithmic tangent dTau/dF checked against a central-difference approximation.
      ...
      for ( int i = 0; i < 3; ++i ) {
        for ( int I = 0; I < 3; ++I ) {
          ...
          throwExceptionOnFailure( std::abs( dTau_dF_numeric - tangent.dTau_dF( i, I, k, K ) ) < 1e-4,
                                   "N-3: Algorithmic tangent mismatch at (i,I,k,K)=(" + std::to_string( i ) + "," +
...
// Test N-4: objectivity (Q*F) and isotropy (F*Q).
void testRotation()
{
  Tensor33d inputF( 0.0 );
  inputF( 0, 0 ) = 1.01;
  ...
  for ( int phi_deg = 0; phi_deg <= 180; phi_deg += 30 ) {
...

    {
      ...
      throwExceptionOnFailure( checkIfEqual( response.tau, stressRotated, 1e-8 ),
                               "N-4a: Objectivity test failed (phi_deg=" + std::to_string( phi_deg ) + ")" );
    }

    {
      ...
      throwExceptionOnFailure( checkIfEqual( response.tau, responseUnrotated.tau, 1e-8 ),
                               "N-4b: Isotropy test failed (phi_deg=" + std::to_string( phi_deg ) + ")" );
    }
  }
}
