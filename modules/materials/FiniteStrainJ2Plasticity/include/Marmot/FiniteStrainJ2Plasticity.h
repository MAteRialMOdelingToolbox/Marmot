...
public:
  ...
  FiniteStrainJ2Plasticity(const double *materialProperties,
                           int nMaterialProperties, int materialLabel);
  ...
  int getNumberOfRequiredStateVars() {
    return FiniteStrainJ2PlasticityStateVarManager::layout.nRequiredStateVars;
  }

  class FiniteStrainJ2PlasticityStateVarManager
      : public MarmotStateVarVectorManager {
  ...
  std::tuple<double, Tensor33d, double> yieldFunction(const Tensor33d &Fe,
                                                      const double betaP) {
    ...
    std::tie(f, df_dMandel, d2f_dMandel_dMandel, df_dBetaP) =
        yieldFunctionFromStress(mandelStress, betaP);
  ...
  T yieldFunctionFromStress(const Tensor33t<T> &mandelStress, const T betaP) {
    const Tensor33t<T> dev =
        ContinuumMechanics::Micropolar::GeneralizedInvariants::deviatoric(
            mandelStress);
  ...
  std::tuple<double, Tensor33d, Tensor3333d, double>
  yieldFunctionFromStress(const Tensor33d &mandelStress, const double betaP) {
    Tensor33d dev =
        ContinuumMechanics::Micropolar::GeneralizedInvariants::deviatoric(
            mandelStress);
    ...
    Tensor3333d d2Rho_dMandel_dMandel =
        -1.0 / std::pow(rho, 3) * outer(dev, dev) +
        1. / rho *
            (einsum<ik, jl, to_ijkl>(Spatial3D::I, Spatial3D::I) -
             1. / 3 *
                 einsum<ij, IK, IL, to_ijKL>(Spatial3D::I, Spatial3D::I,
                                             Spatial3D::I));
  ...
  template <typename T>
  std::tuple<T, Tensor33t<T>, T>
  yieldFunctionFromStressFirstOrderDerived(const Tensor33t<T> &mandelStress,
                                           const T betaP) {
    Tensor33t<T> dev =
        ContinuumMechanics::Micropolar::GeneralizedInvariants::deviatoric(
            mandelStress);
    ...
    const Tensor33t<T> dRho_dMandel =
        multiplyFastorTensorWithScalar(dev, 1. / rho);

    const Tensor33t<T> df_dMandel =
        multiplyFastorTensorWithScalar(dRho_dMandel, T(1. / fy));
  ...
  template <typename T> T computeBetaPOnly(const T alphaP) {
  ...
  std::tuple<double, double> g(const Tensor33d &Fe_trial,
                               const Tensor33d &df_dS, double dLambda,
                               double betaP) {
    ...
    Tensor3333d dFe_ddFp =
        einsum<iI, iJKL>(Fe_trial, einsum<LI, JK, to_IJKL>(-dFpInv, dFpInv));

    std::tie(f, df_dFe, df_dBetaP) =
        yieldFunction(Tensor33d(Fe_trial % dFpInv), betaP);

    double df_ddLambda =
        einsum<IJ, IJKL, KL>(df_dFe, dFe_ddFp, ddFp_ddLambda).toscalar();
  ...
  Tensor33t<T> computeReturnMappingDirection(Tensor33t<T> Fe, T betaP) {
    ...
    double f, df_dBetaP;
    Tensor33d df_dS;
    std::tie(f, df_dS, df_dBetaP) =
        yieldFunctionFromStress(mandelStressTrial, betaP);

    return df_dS;
  }

  template <typename T>
  std::tuple<Tensor33d, Tensor33d> computePlasticIncrement(Tensor33t<T> df_dS,
                                                           T dLambda) {

    Tensor33d dGp = multiplyFastorTensorWithScalar(df_dS, dLambda);
    Tensor33d dFp;
    Tensor3333d ddFp_ddGp;
    std::tie(dFp, ddFp_ddGp) = ContinuumMechanics::Micropolar::Plasticity::
        FlowIntegration::FirstOrderDerived::exponentialMap(dGp);
  ...
  VectorXt<T> computeResidualVector(const VectorXt<T> &X,
    ...
    std::tie(f, df_dMandel, df_dBetaP) =
        yieldFunctionFromStressFirstOrderDerived(mandelStress, betaP);

    const Tensor33t<T> dGp =
        multiplyFastorTensor33WithScalar(df_dMandel, dLambda);
    const Tensor33t<T> dFp = ContinuumMechanics::Micropolar::Plasticity::
        FlowIntegration::exponentialMap(dGp);
  ...
  computeResidualVectorAndTangent(const Eigen::VectorXd &X,
                                  const Tensor33d &FeTrial,
                                  const double alphaPTrial) {
    ...
    Tensor3333d ddGp_dFe =
        dLambda * einsum<ijmn, mnkL>(d2f_dMandel_dMandel, dMandel_dFe);
    ...
    Tensor3333d dFeTrial_dFe =
        einsum<iI, IJKL>(Fe, ddFp_dFe) +
