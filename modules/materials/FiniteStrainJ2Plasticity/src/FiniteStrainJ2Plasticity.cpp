...
FiniteStrainJ2Plasticity::FiniteStrainJ2Plasticity(
    const double *materialProperties, int nMaterialProperties,
    int materialLabel)
    : MarmotMaterialFiniteStrain(materialProperties, nMaterialProperties,
                                 materialLabel),
      K(materialProperties[0]), G(materialProperties[1]),
      fy(materialProperties[2]), fyInf(materialProperties[3]),
      eta(materialProperties[4]), H(materialProperties[5]),
      implementationType(materialProperties[6]) {}

void FiniteStrainJ2Plasticity::computeStress(
    ConstitutiveResponse<3> &response, AlgorithmicModuli<3> &tangents,
    const Deformation<3> &deformation, const TimeIncrement &timeIncrement) {
  switch (implementationType) {
  case 0:
    computeStressWithScalarReturnMapping(response, tangents, deformation,
                                         timeIncrement);
    break;
  case 1:
    computeStressWithFullReturnMapping(response, tangents, deformation,
                                       timeIncrement);
    break;
  case 2:
  ...
  case 3:
    ...
    break;
  case 4:
    computeStressCSDA(response, tangents, deformation, timeIncrement);
    break;
  default:
    throw std::invalid_argument("implementation type not supported");
  };
}
void FiniteStrainJ2Plasticity::computeStressWithScalarReturnMapping(
    ConstitutiveResponse<3> &response, AlgorithmicModuli<3> &tangents,
    const Deformation<3> &deformation, const TimeIncrement &timeIncrement) {
  throw std::invalid_argument("not implemented yet");
}
void FiniteStrainJ2Plasticity::computeStressWithFullReturnMapping(
    ConstitutiveResponse<3> &response, AlgorithmicModuli<3> &tangents,
    const Deformation<3> &deformation, const TimeIncrement &timeIncrement) {
  ...
  if (isYielding(FeTrial, betaP)) {
    ...
    while (R.norm() > 1e-12 || dX.norm() > 1e-12) {
      ...
      X += dX;
      std::tie(R, dR_dX) =
          computeResidualVectorAndTangent(X, FeTrial, alphaPOld);
      counter += 1;
    }
    ...
    std::tie(Ce, dCe_dFe) =
        DeformationMeasures::FirstOrderDerived::CauchyGreen(Fe);

    // compute energy density, first and second partial derivatives wrt Cauchy
    // Green deformation
    std::tie(psi_, dPsi_dCe, d2Psi_dCedCe) =
        EnergyDensityFunctions::SecondOrderDerived::PenceGouPotentialB(Ce, K,
                                                                       G);
    // compute Kirchhoff stress
    Tensor33d PK2 = 2. * dPsi_dCe;
    Tensor3333d dTau_dPK2, dTau_dFe_partial;
    std::tie(response.tau, dTau_dPK2, dTau_dFe_partial) =
        StressMeasures::FirstOrderDerived::KirchhoffStressFromPK2(PK2, Fe);
    response.rho = 1.0;
    ...
    dYdDeformation.block<9, 9>(0, 0) =
        mM9d(Tensor3333d(einsum<IK, JL, to_IJKL>(
                             Spatial3D::I, transpose(Fastor::inverse(FpOld))))
                 .data())
            .transpose();
    MatrixXd dXdDeformation = dR_dX.colPivHouseholderQr().solve(dYdDeformation);

    Tensor3333d dFe_dF = Tensor3333d(
        Matrix9d(dXdDeformation.block<9, 9>(0, 0).transpose()).data());
     ...
     * dTau_dF_partial; */
    tangents.dTau_dF = einsum<IJKL, KLMN>(dTau_dPK2, dPK2_dF) +
                       einsum<ijKL, KLMN>(dTau_dFe_partial, dFe_dF);
  } else {
    ...
    // compute tangent operator
    Tensor3333d dPK2_dFe = einsum<ijKL, KLMN>(2. * d2Psi_dCedCe, dCe_dFe);
    Tensor3333d dFe_dF = einsum<IK, JL, to_IJKL>(
        Spatial3D::I, transpose(Fastor::inverse(FpOld)));
    Tensor3333d dPK2_dF = einsum<ijKL, KLMN>(dPK2_dFe, dFe_dF);
...
void FiniteStrainJ2Plasticity::computeStressFDAF(
    ConstitutiveResponse<3> &response, AlgorithmicModuli<3> &tangents,
    const Deformation<3> &deformation, const TimeIncrement &timeIncrement) {
  ...
  if (isYielding(FeTrial, betaP)) {
    ...
    try {
      ...
      throw std::runtime_error("return mapping failed: " +
                               std::string(e.what()));
    }
    /* std::cout << "inner newton iters: " << counter << std::endl; */
    ...
    func_type computeS = [&](const Tensor33d &Ce_) {
      const auto [_psi, _dPsi_dCe] =
          EnergyDensityFunctions::FirstOrderDerived::PenceGouPotentialB(Ce_, K,
                                                                        G);
      return _dPsi_dCe;
    };

    d2Psi_dCedCe =
        NumericalAlgorithms::Differentiation::TensorToTensor::forwardDifference(
            computeS, Ce);
    std::tie(psi_, dPsi_dCe) =
        EnergyDensityFunctions::FirstOrderDerived::PenceGouPotentialB(Ce, K, G);
    // compute Kirchhoff stress
    Tensor33d PK2 = 2. * dPsi_dCe;
    Tensor3333d dTau_dPK2, dTau_dFe_partial;
    std::tie(response.tau, dTau_dPK2, dTau_dFe_partial) =
...
void FiniteStrainJ2Plasticity::computeStressFDAC(
    ConstitutiveResponse<3> &response, AlgorithmicModuli<3> &tangents,
    const Deformation<3> &deformation, const TimeIncrement &timeIncrement) {
  ...
  if (isYielding(FeTrial, betaP)) {
    ...
    d2Psi_dCedCe =
        NumericalAlgorithms::Differentiation::TensorToTensor::centralDifference(
            computeS, Ce);
    std::tie(psi_, dPsi_dCe) =
        EnergyDensityFunctions::FirstOrderDerived::PenceGouPotentialB(Ce, K, G);
    // compute Kirchhoff stress
...
void FiniteStrainJ2Plasticity::computeStressCSDA(
    ConstitutiveResponse<3> &response, AlgorithmicModuli<3> &tangents,
    const Deformation<3> &deformation, const TimeIncrement &timeIncrement) {
  ...
  if (isYielding(FeTrial, betaP)) {
    ...
    /* R     = computeResidualVector( X, FeTrial, alphaP ); */
    std::tie(R, dR_dX) =
        NumericalAlgorithms::Differentiation::Complex::forwardDifference(
            [&](const VectorXcd &X_) {
    ...
    // compute energy density, first and second partial derivatives wrt Cauchy
    // Green deformation
    using func_type = std::function<Tensor33t<complexDouble>(
        const Tensor33t<complexDouble> &)>;
    ...
    d2Psi_dCedCe = NumericalAlgorithms::Differentiation::Complex::
        TensorToTensor::forwardDifference(computeS, Ce);
    std::tie(psi_, dPsi_dCe) =
        EnergyDensityFunctions::FirstOrderDerived::PenceGouPotentialB(Ce, K, G);
    // compute Kirchhoff stress
    Tensor33d PK2 = 2. * dPsi_dCe;
...
void FiniteStrainJ2Plasticity::assignStateVars(double *stateVars_,
                                               int nStateVars) {
  ...
  this->stateVars =
      std::make_unique<FiniteStrainJ2PlasticityStateVarManager>(stateVars_);
}

void FiniteStrainJ2Plasticity::initializeYourself() { stateVars->Fp.eye(); }
} // namespace Marmot::Materials
