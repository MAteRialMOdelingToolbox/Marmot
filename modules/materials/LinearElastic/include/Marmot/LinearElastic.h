  ...
  protected:
    ...
    enum class Type {
      ...
      TransverseIsotropic = 11, /**< Number of materialProperties equals 11.
                                 ...
                                 *   6 parameters correspond to two normal vectors.
                                 *   The first vector gives the anisotropic direction (direction \f$x_1\f$).
                                 *   The second vector (direction \f$x_2\f$) is the first of the two needed vectors for
                                 * the isotropic plane (directions \f$x_2\f$ and \f$x_3\f$). The \f$x_3\f$ axis is
                                 * obtained as the normalized cross product of the two vectors.
                                 */
      Orthotropic = 15          /**< Number of materialProperties equals 15.
                                 * 	 9 parameters describe the material behaviour and
                                 *   6 parameters are the components of two vectors that define the principal material
                                 * directions.          The two vectors correspond to the directions \f$x_1\f$ and \f$x_2\f$.          The
                                 * third          principal axis \f$x_3\f$ is the unit normal to the plane spanned by those
    ...
    /** \f$E_1\f$ represents the Young's modulus effective in the direction of the \f$x_1\f$-axis of the user-defined
     * local coordinate system. It is a reference variable to #materialProperties[0].
     */
    const double& E1;

    /// @brief Young's modulus in \f$x_2\f$-direction
    /** \f$E_2\f$ represents the Young's modulus effective in the direction of the \f$x_2\f$-axis of the user-defined
     * local coordinate system. In case of isotropic behavior it is set to \f$E_1\f$ and does not need to be specified.
     * Otherwise it is a reference variable to #materialProperties[1].
     */
    const double& E2;

    /// @brief Young's modulus in \f$x_3\f$-direction
    /** \f$E_3\f$ represents the Young's modulus effective in the direction of the \f$x_3\f$-axis of the user-defined
    ...
    /** \details \f$G_{12}\f$ represents the shear modulus effective between the \f$x_1\f$- and \f$x_2\f$-axes of the
     * user-defined local coordinate system. In case of isotropic behavior it does not need to be specified and is set
     * to \f$\tfrac{E_1}{2 \cdot (1 + \nu_{12})}\f$. Otherwise, depending on the anisotropic type it is a reference
     * variable to:
    ...
    /** \details \f$G_{23}\f$ represents the shear modulus effective between the \f$x_2\f$- and \f$x_3\f$-axes of the
     * user-defined local coordinate system. For isotropic behavior it is set to #G12. For transverse-isotropic behavior
     * it is set to \f$\tfrac{E_2}{2 \cdot (1 + \nu_{23})}\f$. For orthotropic behavior it is a reference variable to
     * #materialProperties[7].
     */
    const double G23;

    /// @brief Shear modulus between axes \f$x_1\f$ and \f$x_3\f$
    /** \details \f$G_{13}\f$ represents the shear modulus effective between the \f$x_1\f$- and \f$x_3\f$-axes of the
