...
namespace Marmot::Materials {
  ...
  KelvinChainInterface::KelvinChainInterface( const double* materialProperties,
      ...
      h( materialProperties[2] )
  {
    if ( nMaterialProperties < 8 ) {
      throw std::invalid_argument( "KelvinChainInterface requires at least 8 material properties." );
    }
    ...
    bulkMaterial = std::make_unique< LinearViscoelasticPowerLaw >( bulkMaterialProperties.data(),
                                                                   static_cast< int >( bulkMaterialProperties.size() ),
                                                                   materialNumber );

    stateLayout.add( "kelvinStateVars", bulkMaterial->getNumberOfRequiredStateVars() );
    stateLayout.finalize();
  }

  KelvinChainInterface::~KelvinChainInterface() = default;

  void KelvinChainInterface::computeStress( State&               state,
                                            Tangents&            tangents,
                                            ...
                                            const TimeIncrement& timeIncrement )
  {
    ...
    const Marmot::Vector6d stressVoigt     = ( 1. / h ) *
                                         Marmot::ContinuumMechanics::VoigtNotation::stressToVoigt( scaledStressSym );

    Marmot::Matrix6d                   tangent         = Marmot::Matrix6d::Zero();
    double*                            kelvinStateVars = stateLayout.getPtr( state.stateVars, "kelvinStateVars" );
    MarmotMaterialHypoElastic::state3D bulkState{ stressVoigt, 0.0, 0.0, kelvinStateVars };
    ...
    const Eigen::Matrix< double, 3, 3, Eigen::RowMajor >
      scaledStress = h * Marmot::ContinuumMechanics::VoigtNotation::voigtToStress( bulkState.stress );
    surfaceStress  = Tensor33d( scaledStress.data() );
    force          = ( 1. / h ) * Fastor::einsum< ij, j, to_i >( surfaceStress, normal );
  }

  void KelvinChainInterface::initializeYourself( double* stateVars, int )
  {
    bulkMaterial->initializeYourself( stateLayout.getPtr( stateVars, "kelvinStateVars" ),
                                      bulkMaterial->getNumberOfRequiredStateVars() );
  }

  double KelvinChainInterface::getDensity()
  {
