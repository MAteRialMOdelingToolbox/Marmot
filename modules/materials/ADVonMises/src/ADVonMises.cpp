...
ADVonMises::ADVonMises(const double *materialProperties,
                       int nMaterialProperties, int materialNumber)
    : MarmotMaterialHypoElasticAD::MarmotMaterialHypoElasticAD(
          materialProperties, nMaterialProperties, materialNumber),
      E(materialProperties[0]), nu(materialProperties[1]),
      yieldStress(materialProperties[2]), HLin(materialProperties[3]),
      deltaYieldStress(materialProperties[4]), delta(materialProperties[5]),
      G(E / (2. * (1. + nu))) {
...
void ADVonMises::assignStateVars(double *stateVars, int nStateVars) {
  if (nStateVars < getNumberOfRequiredStateVars())
    throw std::invalid_argument(MakeString() << __PRETTY_FUNCTION__
                                             << ": Not sufficient stateVars!");

  managedStateVars =
      std::make_unique<ADVonMisesModelStateVarManager>(stateVars);
...
void ADVonMises::computeStressAD(autodiff::dual *stress,
  ...
  const Matrix6d Cel =
      ContinuumMechanics::Elasticity::Isotropic::stiffnessTensor(E, nu);
  ...
  if (Math::makeReal(f(rhoTrial, kappa)) >= 0.0) {
    ...
    while (abs(g((double)rhoTrial, kappa, (double)dKappa)) >
           ADVonMisesConstants::innerNewtonTol) {
    ...
    const Vector6dual n =
        ContinuumMechanics::VoigtNotation::IDev * trialStress / rhoTrial;
