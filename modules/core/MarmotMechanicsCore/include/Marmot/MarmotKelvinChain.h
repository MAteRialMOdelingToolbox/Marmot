...
/**
     * Evaluates the Post-Widder formula for deriving the equivalent retardance \f$L_\kappa(\tau)\f$ of the discrete Kelvin Chain model, given the continuous description of the compliance \f[\Phi(t) = \int_{\tau=0}^\infty L(\tau)\left(1-e^{-\frac{t}{\tau}}\right)d(\ln\tau)\f]  
     ...
     * @tparam k \f$k\f$ integer defining the order of differentiation that corresponds to the \f$L_k\f$ term in the sequence of the retardance approximation 
...
/**
     ...
     * It is used in order to avoid indegrating over the origin of the frequency domain where \f$\int_0^{\tau_0}d\ln(\tau)\f$ is singular. 
     * @tparam k \f$k\f$ integer defining the order of differentiation for the \f$k-th\f$ approximation of the retardance. 
     * @param[in] phi compliance function whose discrete approximation is sought.
     * @param[in] tauMin the minimum retardation time that is required to be of the same order or smaller than the total time of the analysis.
...
/**
     ...
     * @tparam k \f$k\f$ integer defining the order of differentiation for the \f$k-th\f$ approximation of the retardance. 
     * @param[in] phi compliance function whose discrete approximation is sought
     * @param[in] retardationTimes the minimum retardation time that is required to be of the same order or smaller than the total time of the analysis 
...
/**
     * For the given time increment \f$\Delta t_k\f$ this function updates the visco-elastic state variables according to the update rule:
     * \f[\varepsilon^{ev,\mu,k+1}_{ij} = \frac{\lambda^{\mu k}}{E_\mu}C^\nu_{ijkl}\Delta\sigma_{kl}+\beta^{\mu ,k}\varepsilon^{ev,\mu, k}_{ij}\f]
     ...
     * @param[in,out] stateVars the \f$[6\times \mu]\f$ matrix that contains the viscoelastic strain update for each unit of the Kelvin chain.
     * @param[in] dStress the \f$[6\times 1]\f$ vector of the total stress increment.
     * @param[in] unitComplianceMatrix the [6\times 6] compliance matrix of the material with unit compliance and given Poisson coeffiscient. 
     */
...
/**
     * For the given time increment \f$\Delta t_k\f$ this function updates the visco-elastic state variables according to the update rule:
     * \f[\overline{J}^k=\sum_{\mu=1}^M\frac{1-\lambda^{\mu,k}}{E^\mu}\f]
     *\f[\Delta\varepsilon^{'',k}_{ij} = \sum_{\mu=1}^M\left(1-\beta^{\mu,k}\right)\varepsilon^{ev,\kappa}_{ij}\f]
     * @param[in] dT the time increment. 
     * @param[in] elasticModuli vector containing the elastic moduli of ech Kelvin unit in the Kelvin chain. 
     * @param[in] retardationTimes vector containing the retardation time for each Kelvin unit in the Kelvin chain.  
     * @param[in] stateVars the \f$[6\times \mu]\f$ matrix that contains the viscoelastic strain update for each unit of the Kelvin chain.
